...
public interface BranchMilkTeaRepository extends JpaRepository<BranchMilkTea, Integer> {
	...
	@Query("SELECT bmt FROM BranchMilkTea bmt WHERE bmt.branch.branchID = :branchID AND bmt.milkTea.milkTeaID = :milkTeaID")
	BranchMilkTea findByBranchIDAndMilkTeaID(@Param("branchID") int branchID, @Param("milkTeaID") int milkTeaID);
